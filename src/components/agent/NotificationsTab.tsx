...
import { Bell, Check, AlertCircle, Info, Calendar, Clock, Building2, X } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        // Process data to handle agency_invitation notifications correctly
        const processedData = (data || []).map(notification => {
          // Check if it's an agency invitation notification
          if (notification.type === 'agency_invitation') {
            ...
            return { ...notification, agency_id: agencyId, link_url: null };
          }
          return notification;
        });
      ...
      <div className="space-y-4">
        {filteredNotifications.length > 0 ? (
          filteredNotifications.map(notification => {
            ...
            console.log('Rendering notification:', notification.id, 'Type:', notification.type, 'Agency ID:', notification.agency_id);
            
            return (
              <div 
                key={notification.id}
                role="presentation"
                className={`bg-white rounded-lg shadow-sm border ${notification.is_read ? 'border-gray-200' : 'border-l-4 border-l-black border-gray-200'} hover:shadow-md transition-all duration-200`}
                ...
                <div className="p-4">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      {/* Regular link for non-agency_invitation with valid link_url */}
                      {!isAgencyInvite && notification.link_url && !notification.link_url.includes('agency_id=') && (
                      ...
                      {/* Agency Invitation Details - Force display for debugging */}
                      {isAgencyInvite && notification.agency_id && (
                        <>
                          <div className="mt-2 bg-yellow-50 p-1 rounded text-xs text-yellow-600">
                            Debug: Agency ID: {notification.agency_id}
                          </div>
